...
function adicionaClientes (cliente){    
    ...
    return 'cadastro deve ser tipo string.';
}
}
//  console.log(adicionaClientes('João'));
//  console.log(adicionaClientes(123));
...
function excluirCliente (cliente){
    if(typeof cliente === 'string'){
        const position = clientesTrybeBank.indexOf(cliente)
        clientesTrybeBank.splice(position,1)
    }else{
        console.log('Erro, cadastro deve ser tipo string')
    }
}
excluirCliente('John')
console.log(clientesTrybeBank);